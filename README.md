...
[最新版本 v2.0.13](https://github.com/chensl139-ok/switchboard-ai-router/releases/tag/v2.0.13) · [更新记录](CHANGELOG.md) · [API 接入说明](API.md) · [租户与权限](TENANCY.md)
...
```sh
docker pull ghcr.io/chensl139-ok/switchboard-ai-router:2.0.13
docker run -d --name switchboard-ai-router \
  ...
  -v "$PWD/data:/app/data" \
  ghcr.io/chensl139-ok/switchboard-ai-router:2.0.13
```

若使用 Release 中的离线镜像包：

```sh
gzip -dc switchboard-ai-router-v2.0.13-oci.tar.gz | docker load
SWITCHBOARD_VERSION=2.0.13 docker compose up -d
```
