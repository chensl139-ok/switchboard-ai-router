...
5. 从 `/healthz` 验证服务；再初始化所有者账户并登录产品配置服务商，使用一次真实生成验收。
...
首次启动使用 ADMIN_TOKEN 完成所有者账户初始化；以后使用邮箱与密码登录。原数据保留在默认租户。不要复制旧会话 token 作为业务 Key。公网部署请设置 COOKIE_SECURE=true。详情见 [租户与权限说明](TENANCY.md)。
