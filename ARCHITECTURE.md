...
| `public/analytics.js` | 用量摘要、连续日期趋势与模型明细 |
| `public/style.css`、`public/interface.css` | 兼容现有组件样式；新界面的主题变量、布局与深色模式集中在 `interface.css` |

调用流程：客户端请求 → 鉴权/限额 → 路由选模型与密钥渠道 → 上游协议适配 → 安全网络访问 → 统一响应/用量记录。实验室只调用已有网关接口，不接触服务商密钥；对比中每个模型单独发请求，不共享对话上下文，最多并发两个。

控制台导航由 `public/app.js` 统一判定角色权限和当前页；页面模块只负责自身内容。页面级标签通过事件委托处理，异步刷新内容时无需重新绑定。浅色和深色主题共享 `interface.css` 的语义变量，新增组件应优先使用这些变量，避免继续在 `style.css` 中叠加固定颜色。
