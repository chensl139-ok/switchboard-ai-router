...
export class ApiKeyStore {
 constructor(dir,{now=()=>Date.now()}={}){
  this.file=path.join(dir,'api-keys.json');this.now=now;
  this.state=existsSync(this.file)?JSON.parse(readFileSync(this.file,'utf8')):{legacyEnabled:true,keys:[]};
 }
 ...
 status(key){if(!key.enabled)return 'disabled';const now=this.now();if(key.startsAt&&now<Date.parse(key.startsAt))return 'scheduled';if(key.expiresAt&&now>=Date.parse(key.expiresAt))return 'expired';return 'active';}
 ...
 list(){return {legacyEnabled:this.state.legacyEnabled,keys:this.state.keys.map(k=>this.publicKey(k))};}
 create(input){const policy=this.validate(input);if(this.state.keys.length>=200)throw error('最多创建 200 个 Key');
  const id=randomBytes(10).toString('hex'),token=`srk_${id}_${randomBytes(32).toString('hex')}`;
 ...
 update(id,input){const policy=this.validate(input);return this.mutate(()=>{const key=this.state.keys.find(k=>k.id===id);if(!key)throw error('Key 不存在',404);Object.assign(key,policy);return this.publicKey(key);});}
 toggle(id,enabled){if(typeof enabled!=='boolean')throw error('启用状态无效');return this.mutate(()=>{const key=this.state.keys.find(k=>k.id===id);if(!key)throw error('Key 不存在',404);key.enabled=enabled;return this.publicKey(key);});}
 ...
 authenticate(token){
  ...
  if(status!=='active')throw error({disabled:'API Key 已停用',scheduled:'API Key 尚未生效',expired:'API Key 已过期'}[status],403);
  return key.id;
 }
 admit(id){
  return this.mutate(()=>{
