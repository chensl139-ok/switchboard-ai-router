 ...
 async function route(input,{signal:clientSignal,onChunk,onNativeEvent,onStreamComplete,clientKind='chat',requestIdentifier,apiKeyId,actorId=null,transport='http'}={}){
  ...
  for(const p of candidates){
   ...
   try{
    ...
    if(input.stream){
     const tokens=await consumeSSE(resp,p.protocol,p.model,async chunk=>{assertThinkingDisabled(input,chunk.choices?.[0]?.delta);committed=true;await onChunk(chunk);},signal,value=>{usage=normalizeUsage({prompt_tokens:value.inputTokens,completion_tokens:value.outputTokens,total_tokens:value.totalTokens});usage.known=usage.known&&value.known;},onNativeEvent&&p.protocol==='anthropic'?async event=>{if(input.thinking_mode==='disabled')assertThinkingDisabled(input,{reasoning_content:event.delta?.thinking||event.content_block?.thinking});committed=true;await onNativeEvent(event);}:null);
