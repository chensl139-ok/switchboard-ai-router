import {renderPlayground,stopPlayground} from './playground.js';
import {renderRouting,routeModes} from './routing.js';
import {renderApiKeys} from './api-keys.js';
let modelList=[],modelEpoch=0;
let token=sessionStorage.getItem('router-token')||'',state,tab='overview';
const $=s=>document.querySelector(s),esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const names={overview:'路由控制台',providers:'服务商管理',playground:'模型实验室',logs:'请求日志',api:'API 接入',keys:'API Key 管理',routing:'路由策略'};
...
function render(){
 ...
 $('#content').innerHTML=html;if(tab==='routing')renderRouting({state,api,esc,toast,updated:s=>{state=s;render();}});if(tab==='keys')void renderApiKeys({api,esc,toast});if(tab==='playground')renderPlayground({state,token,esc,refresh:async()=>{try{state=await api('/api/state')}catch(e){toast(e.message)}}});
}
...
$('#logout').onclick=()=>{sessionStorage.removeItem('router-token');token='';state=null;stopPlayground();$('#content').innerHTML='';$('#login').showModal()};
