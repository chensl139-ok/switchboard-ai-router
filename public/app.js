...
document.querySelector('#provider-form [name="modelProtocols"]').closest('label').insertAdjacentHTML('afterend','<label>模型计费渠道（每行：模型 ID = subscription / metered；未列出默认 Subscription）<textarea name="modelChannels" rows="3" placeholder="claude-haiku-4-5 = metered"></textarea></label>');
...
document.querySelector('#fetch-models').insertAdjacentHTML('beforebegin','<label>查询模型的密钥渠道<select name="modelChannelLookup"><option value="subscription">Subscription</option><option value="metered">Metered</option></select></label>');
...
const ready=p=>p.enabled&&p.model&&(p.modelChannels?.[p.model]==='metered'?p.hasMeteredKey:p.hasKey);
...
function render(){
 ...
 if(tab==='providers')html=heading('服务商管理','连接、配置并切换你的模型服务商。',isManager()?'<button id="add-provider" class="primary">＋ 添加服务商</button>':'')+`<div class="filter-bar"><label class="search-field"><span class="sr-only">搜索服务商或模型</span><input type="search" id="provider-search" placeholder="搜索服务商、模型名称或 ID…"></label><select id="provider-filter" aria-label="服务商状态"><option value="">全部状态</option><option value="true">已启用且配置完整</option><option value="false">待配置 / 未启用</option></select><span id="provider-count" class="muted"></span></div>`+cards()+`<div id="provider-empty" class="empty" hidden>未找到匹配服务商。试试其他关键词或调整状态筛选。</div>`+'<div class="info">配置服务商时可点击「获取所有模型」并搜索选择；同一服务商可保存多个模型，在卡片下拉框直接切换，共用一份密钥。启用只表示配置完整，实际连通性请在实验室验证。</div>';
...
function edit(id){const p=state.providers.find(p=>p.id===id)||{id:'',name:'',baseUrl:'',model:'',protocol:'openai',priority:50,weight:1,enabled:false};const f=$('#provider-form');f.reset();resetModels();for(const k of ['id','name','baseUrl','model','protocol','priority','weight'])f.elements[k].value=p[k];f.elements.anthropicAuth.value=p.anthropicAuth||'x-api-key';f.elements.models.value=(p.models||[]).join('\n');f.elements.modelProtocols.value=Object.entries(p.modelProtocols||{}).map(([model,protocol])=>`${model} = ${protocol}`).join('\n');f.elements.modelChannels.value=Object.entries(p.modelChannels||{}).map(([model,channel])=>`${model} = ${channel}`).join('\n');f.elements.id.readOnly=!!id;f.elements.enabled.checked=p.enabled;$('#edit-error').textContent='';$('#edit').showModal()}
...
$('#fetch-models').onclick=async()=>{
 ...
 try{
  const out=await api('/api/provider/models',{id:f.elements.id.value,baseUrl:f.elements.baseUrl.value,protocol:f.elements.protocol.value,anthropicAuth:f.elements.anthropicAuth.value,channel:f.elements.modelChannelLookup.value,apiKey:f.elements.apiKey.value,meteredApiKey:f.elements.meteredApiKey.value,clearKey:f.elements.clearKey.checked});
...
$('#provider-form').onsubmit=async e=>{e.preventDefault();const f=e.target,submit=f.querySelector('[type=submit]'),b=Object.fromEntries(new FormData(f));if(submit.disabled)return;submit.disabled=true;submit.textContent='保存中…';b.enabled=f.elements.enabled.checked;b.clearKey=f.elements.clearKey.checked;b.priority=Number(b.priority);b.weight=Number(b.weight);b.models=f.elements.models.value.split('\n').map(m=>m.trim()).filter(Boolean);try{b.modelProtocols={};for(const line of f.elements.modelProtocols.value.split('\n').map(s=>s.trim()).filter(Boolean)){const match=line.match(/^(.+?)\s*=\s*(openai|responses|anthropic)$/);if(!match)throw Error('模型协议格式应为：模型 ID = openai / responses / anthropic');b.modelProtocols[match[1].trim()]=match[2];}b.modelChannels={};for(const line of f.elements.modelChannels.value.split('\n').map(s=>s.trim()).filter(Boolean)){const match=line.match(/^(.+?)\s*=\s*(subscription|metered)$/);if(!match)throw Error('模型渠道格式应为：模型 ID = subscription / metered');b.modelChannels[match[1].trim()]=match[2];}state=await api('/api/provider',b);f.elements.apiKey.value='';f.elements.meteredApiKey.value='';$('#edit').close();render();toast('服务商配置已保存')}catch(e){$('#edit-error').textContent=e.message}finally{submit.disabled=false;submit.textContent='保存配置'}};
