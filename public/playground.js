...
let contextVersion=0,pendingImages=[],history=[],controller=null,settings={target:'auto',transport:'sse',thinking:'auto',showThinking:true,maxTokens:2048,tools:'[]'};
...
export function resetPlayground(){contextVersion++;controller?.abort();history=[];pendingImages=[];settings.target='auto';settings.tools='[]';}
export function renderPlayground({state,token,tenantId,esc,refresh}){
 ...
 $('#lab-image-input').onchange=async e=>{const version=contextVersion;try{for(const file of e.target.files){if(pendingImages.length>=4)throw Error('最多附加 4 张图片');if(!['image/png','image/jpeg','image/gif','image/webp'].includes(file.type)||file.size>4*1024*1024)throw Error('图片需为 PNG/JPEG/GIF/WebP，单张不超过 4 MB');const url=await new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(reader.result);reader.onerror=reject;reader.readAsDataURL(file);});if(version!==contextVersion||!$('#lab-image-input'))return;pendingImages.push({url,name:file.name});}drawImages();}catch(error){if($('#lab-error'))$('#lab-error').textContent=error.message;}finally{e.target.value='';}};
 ...
 $('#lab-clear').onclick=()=>{if(!controller){history=[];pendingImages=[];drawImages();draw();$('#lab-error').textContent='';}};
 ...
 $('#lab-form').onsubmit=async e=>{
  ...
  const prompt=$('#lab-prompt').value.trim();if(!prompt&&!pendingImages.length)return;
  ...
  history.push({role:'user',content:userContent},response);pendingImages=[];drawImages();$('#lab-prompt').value='';$('#lab-error').textContent='';
