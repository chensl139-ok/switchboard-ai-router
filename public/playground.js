...
export function renderPlayground({state,token,tenantId,esc,refresh}){
 ...
 <div class="lab-session-summary" aria-label="当前实验状态"><div><small>运行状态</small><strong class="lab-state" id="lab-state">准备就绪</strong></div><div><small>上下文</small><strong id="lab-turn-count">0 轮</strong></div><div><small>响应方式</small><strong id="lab-transport-label">SSE 流式</strong></div></div>
 ...
 $('#lab-form').onsubmit=async e=>{
  ...
  try{
   ...
   if(settings.transport==='http'){
    ...
    const data=await res.json();if(!res.ok)throw Error(data.error?.message||'调用失败');result=data.choices[0].message;response.label=data.model||response.label;response.totalTokens=data.usage?.total_tokens;
   ...
   Object.assign(response,result);response.durationMs=Date.now()-started;response.status=response.tool_calls?.length?'等待工具结果':'完成';response.thinkingOpen=false;
