export async function renderApiKeys({api,esc,toast}){
 ...
 container.innerHTML=`<div class="heading"><div><div class="eyebrow">ACCESS & USAGE</div><h1>对外 API Key 管理</h1><p>为每个应用单独签发密钥，控制调用次数与有效期。</p></div><button id="new-key" class="primary">＋ 创建 API Key</button></div>
 <div class="panel"><h2>旧版环境令牌</h2><p>环境变量 GATEWAY_TOKEN 当前${data.legacyEnabled?'仍可调用，不受下方各 Key 配额限制':'已停止对外调用'}。管理员令牌仅供管理员及实验室使用，请勿分发。</p><button id="legacy-key">${data.legacyEnabled?'关闭旧版调用令牌':'启用旧版调用令牌'}</button></div>
 <div class="section-title"><h2>应用密钥 · ${data.keys.length}</h2><button id="refresh-keys">↻ 刷新用量</button></div>
 <div class="cards">${data.keys.map(key=>`<article class="card"><div class="card-top"><h3>${esc(key.name)}</h3><span class="tag ${key.status==='active'?'green':''}">${labels[key.status]}</span></div><p class="model">${esc(key.preview)}</p><p>总调用：${quota(key.requests,key.totalLimit)}<br>今日调用：${quota(key.dailyUsed,key.dailyLimit)}<br>每分钟上限：${key.rpmLimit??'不限'}<br>成功 / 失败：${key.successes} / ${key.failures}<br>已知 Token 用量：${key.knownTokens.toLocaleString()}</p><p class="muted">生效：${formatDate(key.startsAt)}<br>到期：${formatDate(key.expiresAt)}<br>最近使用：${key.lastUsedAt?formatDate(key.lastUsedAt):'尚未调用'}</p><div class="card-actions"><button data-edit-key="${key.id}">修改限制</button><button data-toggle-key="${key.id}">${key.enabled?'停用':'启用'}</button></div></article>`).join('')||'<div class="panel empty">尚无应用密钥。点击右上角创建。</div>'}</div>
 ...
 container.querySelector('#legacy-key').onclick=async e=>{e.target.disabled=true;try{await api('/api/keys/legacy',{enabled:!data.legacyEnabled});await reload()}catch(error){toast(error.message);e.target.disabled=false}};
