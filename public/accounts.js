...
export function showLogin(mode='login'){
 ...
 ${mode==='register'?'<label>邀请码<input name="inviteCode" required type="password" autocomplete="off"></label>':''}<button class="primary">${mode==='login'?'登录':mode==='recover'?'重置密码':'创建账户'}</button><p id="account-error" role="alert"></p>
 ...
 dialog.querySelector('form').onsubmit=async e=>{e.preventDefault();const form=e.target,button=form.querySelector('button');button.disabled=true;try{const result=await accountRequest(mode==='setup'?'setup':mode==='register'?'register':mode==='recover'?'password-reset/complete':'login',Object.fromEntries(new FormData(form)));form.reset();if(mode==='recover'){showLogin('login');dialog.querySelector('#account-error').textContent='密码已重置，请使用新密码登录。';}else{await readyHandler(result);dialog.close();}}catch(error){dialog.querySelector('#account-error').textContent=error.message;}finally{button.disabled=false;}};
}
