...
export function renderMediaLab({state,tenantId,esc}){
 const epoch=viewEpoch,root=document.querySelector('#content'),active=()=>epoch===viewEpoch&&root.isConnected,models=state.providers.filter(p=>p.enabled&&p.hasKey&&p.protocol!=='anthropic').flatMap(p=>p.models.map(model=>({id:p.id+'::'+model,model,provider:p.id,name:p.name+' / '+model})));
