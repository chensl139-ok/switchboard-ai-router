...
export function renderMediaLab({state,tenantId,esc,embedded=false}){
 ...
 function updateVisionFields(){
  const vision=$('#media-kind').value==='vision',video=$('#media-vision-type').value==='video';
  $('#media-vision-type-label').hidden=!vision;$('#media-vision-help').hidden=!vision;
 ...
 function resetResult(){$('#media-result').replaceChildren();}
 ...
 function clearSelection(){
  ...
  $('#media-result').innerHTML='<div class="media-placeholder"><span>＋</span><b>结果将在这里显示</b><small>选择任务和模型后发起新调用</small></div>';
 ...
 $('#media-form').onsubmit=async event=>{
  ...
  if(kind==='transcription'){const file=$('#media-file').files[0];if(!file||file.size>50*1024*1024){$('#media-error').textContent='请选择不超过 50 MB 的音频';return;}body=new FormData();body.set('model',model);body.set('file',file);route='/v1/audio/transcriptions';}
   ...
   const video=$('#media-vision-type').value==='video';
   const urls=video?[$('#media-vision-video').value.trim()]:$('#media-vision-images').value.split('\n').map(value=>value.trim()).filter(Boolean);
   if(!text.trim()||urls.length<1||urls.length>(video?1:5)||urls.some(value=>!value.startsWith('https://')||!safeURL(value))){$('#media-error').textContent=video?'请填写一个可访问的 HTTPS 视频地址':'请填写 1～5 个可访问的 HTTPS 图片地址，每行一个';return;}
   body=JSON.stringify({model,input:[{role:'user',content:[{type:'input_text',text:text.trim()},...urls.map(value=>video?{type:'input_video',video_url:value}:{type:'input_image',image_url:value})]}],max_output_tokens:1024});route='/v1/responses';
  }
  ...
  try{
   ...
   if(kind==='speech'){
    ...
    if(kind==='vision'){const text=(data.output||[]).filter(item=>item.type==='message').flatMap(item=>item.content||[]).filter(part=>part.type==='output_text').map(part=>part.text||'').join('\n');if(!text)throw Error('视觉理解未返回文本结果');const output=document.createElement('p');output.className='media-transcript';output.textContent=text;$('#media-result').append(output);}
    if(kind==='video'){
