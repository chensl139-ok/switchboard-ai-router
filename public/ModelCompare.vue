...
const props=defineProps<{choices: ExperimentChoice[]; token?: string; tenantId?: string; onSwitch: () => void}>();
...
async function run(){
 ...
 try{await Promise.all([worker(),worker()]);if(request.signal.aborted)for(const item of items.value)if(item.status==='等待中')item.status='已停止';lastRun.value={createdAt:new Date().toISOString(),prompt:prompt.value,maxTokens:maxTokens.value,results:items.value.map(item=>({providerId:item.choice.providerId,model:item.choice.model,protocol:item.choice.protocol,channel:item.choice.channel,status:item.status,...(item.result?{result:item.result}:{}),...(item.error?{error:item.error}:{})}))};}
 finally{if(controller===request)controller=null;busy.value=false;}
}
</script>

<template>
 <div class="heading lab-heading"><div><div class="eyebrow">MODEL PLAYGROUND</div><h1>模型实验室</h1><p>同一问题独立发送给多个模型，比较输出、耗时与用量。</p></div><button class="subtle" @click="switchView">单模型对话</button></div>
 <div class="panel compare-controls"><label>测试问题<textarea v-model="prompt" rows="4" maxlength="10000" placeholder="输入一个需要比较的问题" :disabled="busy"></textarea></label><div class="compare-actions"><label>最大输出 Tokens<input v-model.number="maxTokens" type="number" min="1" max="131072" :disabled="busy"></label><button type="button" :disabled="busy || selected.length>=Math.min(4,choices.length)" @click="addModel">＋ 添加模型</button><button type="button" :disabled="busy || selected.length<=2" @click="removeModel">－ 移除模型</button><button class="primary" type="button" :disabled="busy || choices.length<2" @click="run">开始对比</button><button v-if="busy" type="button" @click="stop">停止</button><button type="button" :disabled="!lastRun" @click="saveResults">导出结果</button></div><p v-if="error" class="lab-error" role="alert">{{error}}</p><p class="muted">每个模型单独计费；最多比较 4 个模型，每次最多并发 2 个请求。问题与结果仅保留在当前页面内存中。</p></div>
 <div class="compare-grid"><article v-for="(id,index) in selected" :key="index" class="panel compare-card"><label>模型 {{index+1}}<select v-model="selected[index]" :disabled="busy" @change="clearResults"><option v-for="choice in choices" :key="choice.id" :value="choice.id">{{choice.label}}</option></select></label><p class="muted">{{choiceMap.get(id)?.protocol}} · {{choiceMap.get(id)?.channel}}</p><div class="compare-result"><template v-if="items[index]?.status==='完成' && items[index].result"><div class="compare-metrics"><span>耗时 {{items[index].result!.elapsedMs}} ms</span><span>输入 {{items[index].result!.usage.input??'未知'}} tokens</span><span>输出 {{items[index].result!.usage.output??'未知'}} tokens</span></div><details v-if="items[index].result!.reasoning"><summary>思考内容</summary><pre>{{items[index].result!.reasoning}}</pre></details><pre class="compare-output">{{items[index].result!.content||'(无文字输出)'}}</pre><p v-if="items[index].result!.toolCalls.length" class="muted">返回 {{items[index].result!.toolCalls.length}} 个工具调用</p></template><p v-else class="muted">{{items[index]?.status||'等待测试'}}{{items[index]?.error?'：'+items[index].error:''}}</p></div></article></div>
</template>
