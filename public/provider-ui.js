...
export function providerCards({state,esc,isManager,manage=false}){
 return `<div class="cards provider-grid">${state.providers.map((provider,index)=>{
  ...
  return `<article data-provider-card data-search="${esc([provider.name,provider.id,...(provider.models||[])].join(' ').toLowerCase())}" data-ready="${ready?'true':'false'}" class="card provider-card ${active?'active':''}"><div class="card-top"><div class="avatar">${esc(provider.name[0])}</div><div class="provider-title"><h3>${esc(provider.name)}</h3><small>${esc(provider.id)}</small></div><span class="provider-rank" aria-label="路由顺序 ${index+1}">${String(index+1).padStart(2,'0')}</span><span class="tag ${ready?'green':''}">${!provider.enabled?'未启用':!ready?'待配置':active?'默认路由':'已启用'}</span></div><div class="provider-health" data-provider-health="${esc(provider.id)}">${healthMarkup(health)}</div><div class="provider-meta"><span>自动协议</span><span>${provider.models?.length||0} 个模型</span><span title="${provider.hasKey&&provider.hasMeteredKey?'主密钥 + 备用密钥，都可参与调用':provider.hasKey||provider.hasMeteredKey?'已保存一把密钥':'尚未配置密钥'}">${provider.hasKey&&provider.hasMeteredKey?'主 + 备用双密钥':provider.hasKey||provider.hasMeteredKey?'已保存密钥':'无密钥'}</span></div><label>当前模型<select data-provider-model="${esc(provider.id)}" ${!provider.models?.length||!isManager?'disabled':''}>${(provider.models?.length?provider.models:['']).map(model=>`<option value="${esc(model)}" ${model===provider.model?'selected':''}>${esc(model||'尚未设置模型')}${model&&!modelCapabilities(model).chat?' · 非对话':''}</option>`).join('')}</select></label><div class="card-actions"><button data-edit="${esc(provider.id)}" ${!isManager?'disabled':''}>配置</button><button data-switch="${esc(provider.id)}" ${!ready||active||!isManager?'disabled':''}>${active?'✓ 默认路由':'设为默认'}</button></div>${controls}</article>`;
 }).join('')}</div>`;
}
