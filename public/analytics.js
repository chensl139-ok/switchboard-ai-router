...
export async function renderLogs({api,esc,state}){
 ...
 for(const id of ['#log-key','#log-model'])root.querySelector(id).onkeydown=e=>{if(e.key==='Enter'&&!e.isComposing){e.preventDefault();search();}};root.querySelector('#log-provider').onchange=root.querySelector('#log-status').onchange=search;root.querySelector('#log-query').onclick=search;root.querySelector('#log-prev').onclick=()=>{page--;void load();};root.querySelector('#log-next').onclick=()=>{page++;void load();};await load();
}
