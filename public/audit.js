...
export async function renderAudit({esc}){
 ...
 async function load(){const current=++version;try{
  ...
  <section class="panel"><div class="panel-title"><div><span class="eyebrow">MEMBER USAGE</span><h2>成员用量</h2></div></div><div class="table-wrap"><table><thead><tr><th>成员</th><th>请求 / 尝试</th><th>最终成功率</th><th>Tokens</th><th>费用估算</th><th>最近使用</th></tr></thead><tbody>${usage.members.map(row=>`<tr><td><strong>${esc(row.name)}</strong><br><small>${esc(row.email||(!row.actorId?'历史调用无法确认成员':''))}${row.role?` · ${esc(roleName(row.role))}`:''}</small></td><td>${Number(row.requests).toLocaleString()} / ${Number(row.attempts).toLocaleString()}</td><td>${row.requests?Math.round(row.requestSuccesses/row.requests*100)+'%':'—'}</td><td>${Number(row.tokens).toLocaleString()}<br><small>输入 ${Number(row.inputTokens).toLocaleString()} · 输出 ${Number(row.outputTokens).toLocaleString()}</small></td><td>${costs(row)}</td><td>${row.lastUsedAt?esc(new Date(row.lastUsedAt).toLocaleString()):'—'}</td></tr>`).join('')}</tbody></table></div>${usage.members.length?'':'<p class="empty">当前周期暂无模型调用</p>'}</section>
