...
export async function streamChat(transport,token,input,onText,signal,tenantId){
 let content='',reasoning='',route=null,usage=null;const toolCalls=new Map();const result=()=>({role:'assistant',content,reasoning_content:reasoning,...(route?{route}:{}),...(usage?{totalTokens:usage.total_tokens}:{}),...(toolCalls.size?{tool_calls:[...toolCalls.values()]}:{})});const receive=chunk=>{const delta=chunk.choices?.[0]?.delta||{};if(typeof delta.content==='string')content+=delta.content;if(typeof delta.reasoning_content==='string')reasoning+=delta.reasoning_content;if(chunk.usage)usage=chunk.usage;for(const tool of delta.tool_calls||[]){const index=tool.index??0;const saved=toolCalls.get(index)||{id:tool.id||'',type:'function',function:{name:tool.function?.name||'',arguments:''}};if(tool.id)saved.id=tool.id;if(tool.function?.name&&!saved.function.name)saved.function.name=tool.function.name;if(tool.function?.arguments)saved.function.arguments+=tool.function.arguments;toolCalls.set(index,saved);}onText(result());};
 if(transport==='ws')return new Promise((resolve,reject)=>{
  ...
  ws.onmessage=e=>{try{const m=JSON.parse(e.data);if(m.type==='ready')ws.send(JSON.stringify({type:'chat',id,input}));
   if(m.type==='delta')receive(m.chunk);if(m.type==='done'){route=m.route||route;usage=m.usage||usage;finished=true;cleanup();resolve(result())}
