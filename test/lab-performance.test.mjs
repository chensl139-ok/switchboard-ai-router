...
test('TTFT 从请求开始到首个输出计时，忽略角色与用量帧',()=>{
 ...
 clock=1320;assert.deepEqual(timing.finish(5,{streamed:true}),{durationMs:320,ttfb:40,ttft:120,blocks:3,tpot:32.5,tps:15.625});
});
...
test('TPOT 以最后输出为终点，不把结束帧和网络收尾时间算进生成',()=>{
 let clock=0;const timing=createLabTiming(()=>clock);
 clock=80;timing.observe(true);clock=180;timing.observe(true);clock=260;timing.observe(true);
 clock=1600;assert.equal(timing.finish(9,{streamed:true}).tpot,22.5);
});
