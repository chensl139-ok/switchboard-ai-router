...
test('跨协议 x-api-key 鉴权、统一配额及模型批量发现隔离',async()=>{
 ...
 try{
  ...
  const discovery=await request('/v1/models/all',null,external);assert.equal(discovery.status,200);assert.equal(discovery.data.errors.length,1);assert.equal(discovery.data.partial,true);assert.equal(discovery.data.data.find(m=>m.id==='first::new-model').callable,false);assert.equal(discovery.data.data.find(m=>m.id==='first::configured').callable,true);assert.equal((await request('/v1/models/discover',null,external)).status,429);const schema=(await request('/v1/openapi.json',null,external)).data;assert.equal(schema.openapi,'3.1.0');for(const path of ['/v1/chat/completions','/v1/responses','/v1/messages','/v1/completions','/v1/messages/count_tokens'])assert.ok(schema.paths[path].post.requestBody);assert.equal((await request('/v1/openapi.json')).status,401);assert.equal(discovery.data.total,2);assert.equal(generated,0);
