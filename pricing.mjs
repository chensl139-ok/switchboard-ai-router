const priceFields=['inputPerMillion','outputPerMillion','cachedInputPerMillion','cacheWritePerMillion','perRequest'];
...
export function validatePrice(input,source='manual'){
 ...
 return {timeZone,periods,currency:input.currency,inputPerMillion:input.inputPerMillion,outputPerMillion:input.outputPerMillion,...(input.cachedInputPerMillion!==undefined?{cachedInputPerMillion:input.cachedInputPerMillion}:{}),...(input.cacheWritePerMillion!==undefined?{cacheWritePerMillion:input.cacheWritePerMillion}:{}),perRequest:input.perRequest||0,source,observedAt,expiresAt};
}
...
export function priceAt(price,now=Date.now()){
 ...
 return period?{...price,...period,cachedInputPerMillion:period.cachedInputPerMillion,cacheWritePerMillion:period.cacheWritePerMillion,periods:[],periodLabel:period.kind==='peak'?'高峰':'空闲'}:{...price,periods:[],periodLabel:'基础'};
}
export function priceEstimate(price,inputTokens,outputTokens,cachedInputTokens=0,now=Date.now(),cacheCreationTokens=0){price=priceAt(price,now);return ((inputTokens-cachedInputTokens-cacheCreationTokens)*price.inputPerMillion+cachedInputTokens*(price.cachedInputPerMillion??price.inputPerMillion)+cacheCreationTokens*(price.cacheWritePerMillion??price.inputPerMillion)+outputTokens*price.outputPerMillion)/1000000+(price.perRequest||0);}
export function usageCost(provider,model,usage,now=Date.now()){
 const price=priceAt(provider.prices?.[model],now);
 if(!usage?.known||usage.cacheUsageInvalid||!price||!usablePrice(price,price.currency,now)||(usage.cachedInputTokens>0&&!Number.isFinite(price.cachedInputPerMillion))||(usage.cacheCreationTokens>0&&!Number.isFinite(price.cacheWritePerMillion)))return {estimatedCost:null,currency:price?.currency||null,priceSource:price?.source||null};
 return {estimatedCost:priceEstimate(price,usage.inputTokens,usage.outputTokens,usage.cachedInputTokens||0,now,usage.cacheCreationTokens||0),currency:price.currency,priceSource:price.source};
}
