   ...
   if(pathname==='/v1/images/generations'){
    ...
    if(sf(p)&&data.response_format==='b64_json'){
     ...
     for(const image of result.data){let url;try{url=new URL(image.url);}catch{throw fail('上游图片地址无效',502);}if(url.protocol!=='https:'||url.username||url.password)throw fail('上游图片地址必须是 HTTPS',502);
      const asset=await fetcher(url.href,{method:'GET',headers:{},redirect:'error',signal,maxResponseBytes:24*1024*1024});if(!asset.ok||!asset.headers.get('content-type')?.startsWith('image/')){await asset.body?.cancel();throw fail('无法获取上游生成图片',502);}const pieces=[];for await(const piece of asset.body){totalBytes+=piece.length;if(totalBytes>24*1024*1024)throw fail('Base64 图片总大小超过 24 MB',502);pieces.push(piece);}encoded.push({b64_json:Buffer.concat(pieces).toString('base64')});
     }result.data=encoded;
    }
