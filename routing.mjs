...
const matchesRoute=(log,provider,model)=>log.model===model&&(log.providerId?log.providerId===provider.id:(!provider.id||log.provider===provider.name));
...
const routeLogsFor=(state,provider,model,usageStore=state?.usageStore)=>{
 if(usageStore?.routeLogs){
  try{return usageStore.routeLogs({id:provider.id,name:provider.name,model});}catch{}
 }
 return (state?.logs||[]).filter(log=>matchesRoute(log,provider,model)).sort((a,b)=>Date.parse(b.time)-Date.parse(a.time));
};
export function routeHealth(state,provider,model,now=Date.now(),usageStore=state?.usageStore){
 const logs=routeLogsFor(state,provider,model,usageStore),latest=logs[0];let consecutiveFailures=0;
...
export function selectRoutes(state,input,options={}){
 return createRouter(state,state?.usageStore||null).selectRoutes(input,options);
}
export function createRouter(state,usageStore){
 ...
 const modelRoutes=(provider,firstModel,now,channelOverride,skipOpen=false)=>[firstModel,...provider.models.filter(model=>model!==firstModel)]
  ...
  .map(({model,index})=>credentialRoutes(provider,model,index===0?(provider.routeReason||'显式起始模型'):'同服务商模型故障转移',channelOverride))
  .reduce((routes,group)=>(routes[0].push(group[0]),routes[1].push(...group.slice(1)),routes),[[],[]]).flat();
 function selectRoutes(input,{sequence=0,now=Date.now(),maxAttempts=state.routing.maxAttempts}={}){
  ...
  if(input.model&&input.model!=='auto'){
   ...
   if(input.upstream_model!==undefined&&split<0&&(state.strategy==='fallback'||input.allow_fallback)){
    const primary=modelRoutes(p,model,now,channelOverride),alternate=primary.find(route=>route.model!==model);
    if(!input.allow_fallback)return primary.slice(0,maxAttempts);
    const others=[...available].filter(route=>route.id!==p.id).sort((a,b)=>a.priority-b.priority||a.id.localeCompare(b.id)).map(route=>modelRoutes(route,route.model,now,undefined,true)).filter(group=>group.length);
    const head=[primary[0],alternate,...others.map(group=>group[0])].filter(Boolean);
    return uniqueRoutes([...head,...primary.filter(route=>!head.includes(route)),...others.flat().filter(route=>!head.includes(route))]).slice(0,maxAttempts);
   }
   return credentialRoutes(p,model,'显式指定服务商与模型',channelOverride).slice(0,maxAttempts);
  }
  ...
  if(['fallback','latency','weighted','rules'].includes(state.strategy)){
   ...
   const first=groups[0]||[],alternate=first.find(route=>route.model!==first[0]?.model);
