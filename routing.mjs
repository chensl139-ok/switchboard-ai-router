...
export function routeHealth(state,provider,model,now=Date.now()){
 const logs=routeLogsFor(state,provider,model),latest=logs[0];let consecutiveFailures=0;
...
export function createRouter(state,usageStore){
 const routeLogs=(provider,model)=>{
  if(usageStore?.routeLogs){
   try{return usageStore.routeLogs({id:provider.id,name:provider.name,model});}catch{}
  }
  return (state.logs||[]).filter(log=>matchesRoute(log,provider,model)).sort((a,b)=>Date.parse(b.time)-Date.parse(a.time));
 };
 const healthOf=(provider,model,now=Date.now())=>{
  const logs=routeLogs(provider,model),latest=logs[0];let consecutiveFailures=0;
  for(const log of logs){if(!retryableStatuses.has(Number(log.status)))break;consecutiveFailures++;}
  const successful=logs.filter(log=>log.status===200&&now-Date.parse(log.time)<3600000).slice(0,20);
  return {circuitOpen:consecutiveFailures>=3&&latest&&now-Date.parse(latest.time)<60000,consecutiveFailures,samples:successful.length,averageLatency:successful.length>=3?successful.reduce((sum,log)=>sum+Number(log.latency||0),0)/successful.length:Infinity};
 };
 ...
 const modelRoutes=(provider,firstModel,now,channelOverride)=>[firstModel,...provider.models.filter(model=>model!==firstModel)]
  .filter((model,index)=>hasModelCredential(provider,model,channelOverride)&&(index===0||!recentlyFailed(provider,model,now)))
 ...
 function selectRoutes(input,{sequence=0,now=Date.now()}={}){
  ...
  if(input.model&&input.model!=='auto'){
   ...
   if(state.strategy==='fallback'&&input.upstream_model!==undefined&&split<0)return modelRoutes(p,model,now,channelOverride).slice(0,state.routing.maxAttempts);
  ...
  else if(!['economy','fallback'].includes(state.strategy))candidates=candidates.filter(p=>!recentlyFailed(p,p.model,now));
  ...
  if(state.strategy==='fallback'){
   const groups=[...candidates.filter(p=>!recentlyFailed(p,p.model,now)).map(p=>credentialRoutes(p,p.model,p.routeReason)),...candidates.flatMap(p=>p.models.filter(model=>model!==p.model&&hasModelCredential(p,model)&&!recentlyFailed(p,model,now)).map(model=>credentialRoutes(p,model,'同服务商模型故障转移')))];
