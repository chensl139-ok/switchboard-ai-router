...
const names={overview:'路由控制台',providers:'服务商管理',playground:'模型实验室',logs:'请求日志',api:'API 接入'};
...
function logs(rows){return rows.length?`<div class="table-wrap"><table><thead><tr><th>请求时间</th><th>服务商 / 模型</th><th>状态</th><th>耗时</th><th>Tokens</th></tr></thead><tbody>${rows.map(l=>`<tr><td>${esc(new Date(l.time).toLocaleString())}</td><td>${esc(l.provider)}<br><small class="muted">${esc(l.model)}</small></td><td><span class="tag ${l.status===200?'green':''}">${l.status}</span></td><td>${l.latency} ms</td><td>${l.tokens.toLocaleString()}</td></tr>`).join('')}</tbody></table></div>`:'<div class="empty">还没有请求记录。配置服务商后，前往模型实验室发起第一次调用。</div>'}
function render(){
 $('#breadcrumb').textContent=names[tab];document.querySelectorAll('nav button').forEach(b=>b.classList.toggle('selected',b.dataset.tab===tab));
 ...
 if(tab==='api')html=heading('一个 API，所有模型','将现有应用的 Base URL 指向此网关，即可在后台切换模型。')+`<div class="panel"><h2>统一调用入口</h2><p>Base URL：<code>${esc(location.origin)}/v1</code></p><p>使用服务器的 GATEWAY_TOKEN 作为 API Key；管理令牌与调用令牌独立。</p><pre>${esc(`curl ${location.origin}/v1/chat/completions \\\n  -H "Authorization: Bearer YOUR_GATEWAY_TOKEN" \\\n  -H "Content-Type: application/json" \\\n  -d '{"model":"auto","messages":[{"role":"user","content":"你好"}],"stream":false}'`)}</pre><div class="info">model="auto" 使用后台路由策略；指定服务商路由 ID 可固定调用。当前支持文本多轮对话、max_tokens 和 temperature，暂不支持流式、工具调用、图像及音频。切换不会迁移第三方聊天产品中的历史记录。</div></div>`;
 $('#content').innerHTML=html;if(tab==='playground')renderMessages();
}
